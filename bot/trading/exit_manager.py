...
Surveille en continu toutes les positions EXECUTED et les ferme si:
  ...
  3. Marché quasi-résolu   → prix > 0.95 (YES résolu) ou < 0.05 (NO résolu)
  4. Durée max dépassée    → position ouverte > MAX_HOLD_HOURS

Tourne en background via asyncio.create_task().
"""
import asyncio
...
class ExitManager:
    """
    Background task de surveillance des positions ouvertes.
    Lance les sorties automatiques selon TP / SL / résolution / durée max.
    """

    TAKE_PROFIT_PCT  = 0.20    # +20% sur le prix d'entrée → Take Profit
    STOP_LOSS_PCT    = 0.30    # -30% sur le prix d'entrée → Stop Loss
    MAX_HOLD_HOURS   = 72      # 72h max de détention
    RESOLVING_THRESH = 0.95    # Prix > 95% → résolution imminente
    CHECK_INTERVAL   = 60      # Fréquence de vérification (secondes)
    ...
    def _calc_pnl_usdc(self, entry: float, current: float, amount: float, side: str) -> float:
        shares = amount / entry if entry > 0 else 0
        return shares * (current - entry) if side.upper() == "BUY" else shares * (entry - current)

    # ------------------------------------------------------------------
    # Traitement d'un cycle
    # ------------------------------------------------------------------
    async def _check_positions(self) -> None:
        with get_db() as db:
            open_trades = (
                ...
                .filter(
                    ...
                    CopiedTrade.skip_reason == "DRY_RUN" if settings.dry_run
        ...
        for trade in open_trades:
            ...
            logger.info(
                f"[EXIT] Closing {trade.side} '{trade.market_question[:40] or trade.token_id[:12]}' "
                f"| {reason} | P&L {pnl:+.2f} USDC"
            )

            # Libère la position dans le RiskManager (met à jour le capital)
            ...
            # TODO (LIVE): envoyer ordre SELL réel via CLOB
            # client.create_market_order(MarketOrderArgs(token_id=..., side=SELL, ...))
            ...
            await self.notifier.send(
                ...
                f"📝 Reason: {reason}\n"
                + ("🟡 <i>DRY RUN</i>" if settings.dry_run else "")
            )
    ...
    async def start(self) -> None:
        """Lance l'exit manager en background."""
        self._running = True
