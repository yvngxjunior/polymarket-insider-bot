"""
Commandes Telegram interactives — PolyInsider Bot v2.8
=======================================================
...
Commandes disponibles:
  ...
  /stop                — arrête le bot proprement
  /setlive             — bascule DRY RUN → LIVE
  /whitelist           — ajoute un wallet à la whitelist
  /blacklist           — ajoute un wallet à la blacklist
  /setcapital [montant]— recalcule les tiered multipliers
  /help                — liste des commandes

FIX BUG-4: /status lit risk_manager.portfolio.total_capital
FIX BUG-5: /pnl lit daily_pnl depuis portfolio.daily_pnl
FIX BUG-6: /whitelist /blacklist persistent via DB
FIX CMD-1: mutations settings via object.__setattr__() (Pydantic v2)
...
class BotCommandHandler:
    ...
    async def _cmd_setlive(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        if not self._pending_setlive:
            ...
            return
        # FIX CMD-1: object.__setattr__() portable Pydantic v1 + v2
        # Avant: settings.__dict__['dry_run'] = False
        # -> ignoré par Pydantic v2 (les attributs sont sur __pydantic_fields_set__)
    ...
    async def _cmd_whitelist(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        with get_db() as db:
            ...
            if wallet:
                ...
                new_w = TrackedWallet(address=address, score=0.80, is_active=True)
                db.add(new_w)
                msg = (
    ...
    async def _cmd_blacklist(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        with get_db() as db:
            ...
            if wallet:
                ...
                new_w = TrackedWallet(address=address, score=0.0, is_active=False)
                db.add(new_w)
                msg = (
    ...
    async def _cmd_setcapital(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        t2 = round(capital, 1)
        t3 = round(capital * 10, 1)
        # FIX CMD-1: object.__setattr__() pour Pydantic v2
        object.__setattr__(settings, "tiered_multipliers", tiered_str)
